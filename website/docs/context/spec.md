...
FDC3 Context data is primarily defined in JSON, using [JSON Schema](https://json-schema.org/) to provide formal definitions for each standardized context type. The JSON Schema definitions should be considered the **source of truth** for the structure and fields of each context type.

However, FDC3 also provides language-specific type definitions (API bindings) that translate the JSON definitions into idiomatic types for each supported language. These bindings may differ from the raw JSON representation in order to provide a more natural developer experience in that language. Applications interacting with FDC3 APIs in a particular language should use the appropriate language binding, while the JSON Schema remains the canonical definition of the wire format exchanged between applications and Desktop Agents.

#### JSON as the Wire Format

The [Desktop Agent Communication Protocol (DACP)](../api/specs/desktopAgentCommunicationProtocol) specifies that context objects are exchanged as JSON-compatible structures. When context is transmitted between applications and Desktop Agents (e.g. via `postMessage` on a `MessagePort`), the data is subject to the browser's [Structured Clone algorithm](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm), which handles serialisation and deserialisation transparently. Applications and Desktop Agent implementations SHOULD pass context objects directly to `postMessage` without additional serialisation (e.g. without calling `JSON.stringify()`).

:::note
The `Convert` classes generated by [quicktype](https://quicktype.io/) from the JSON Schema files are provided as a convenience for validation and type-safe parsing of JSON strings (e.g. when reading context from a file or REST API). They are **not** intended for use when sending or receiving DACP messages over `MessagePort`, where the Structured Clone algorithm handles serialisation.
:::

#### Language-Specific Bindings

Each language binding translates JSON Schema types into idiomatic equivalents for that language. The following table summarises key differences:

| JSON Schema Type | TypeScript/JavaScript | .NET | Go | Java |
|---|---|---|---|---|
| `string` (ISO 8601 date-time) | `string` | `string` | `string` | `String` |
| `string` | `string` | `string` | `string` | `String` |
| `number` | `number` | `double` | `float64` | `Double` / `Number` |
| `integer` | `number` | `int` | `int` | `Integer` / `Number` |
| `boolean` | `boolean` | `bool` | `bool` | `Boolean` |
| `object` (map) | `Record<string, T>` | `IDictionary<string, T>` | `map[string]T` | `Map<String, T>` |
| `array` | `Array<T>` | `IEnumerable<T>` | `[]T` | `List<T>` / `T[]` |

:::note
Date and time fields in context objects are **always string-encoded** in both the JSON wire format and in language bindings. They are **not** converted to native `Date` objects (in JavaScript) or `DateTime` (in .NET) at the context type level. Applications may parse these strings into native date types if needed, but the context objects themselves carry string values conforming to [ISO 8601-1:2019](https://www.iso.org/standard/70907.html). See [Other Field Type Conventions](#other-field-type-conventions) for details.
:::

#### Packages and Source Code

Language-specific context type definitions are available in the following packages:

| Language | Package | Source |
|---|---|---|
| TypeScript/JavaScript | [`@finos/fdc3-context`](https://www.npmjs.com/package/@finos/fdc3-context) | [`packages/fdc3-context`](https://github.com/finos/FDC3/tree/main/packages/fdc3-context) |
| .NET | _Included in the .NET FDC3 implementation_ | See [.NET language binding](https://github.com/finos/FDC3/tree/main/packages/fdc3-dotnet) |
| Go | _Not yet available_ | — |
| Java | Maven `org.finos.fdc3:fdc3-context` | [finos-labs/fdc3-java-api](https://github.com/finos-labs/fdc3-java-api) |

The TypeScript definitions distributed in the `@finos/fdc3-context` NPM module are generated from the JSON Schema files using [quicktype](https://quicktype.io/). Both documentation for fields defined (in the form of a `title` and `description` entry for each field) and examples SHOULD be included in JSON Schema definitions for Context types to ensure that the schema file can serve as a single source of truth, and that code generated from the schema files can also include that documentation.
...
- [`fdc3.chat.initSettings`](ref/chat/ChatInitSettings) ([schema](pathname:///schemas/next/context/chatInitSettings.schema.json))
- [`fdc3.chat.message`](ref/chat/ChatMessage) ([schema](pathname:///schemas/next/context/chatMessage.schema.json))
- [`fdc3.chat.room`](ref/chat/ChatRoom) ([schema](pathname:///schemas/next/context/chatRoom.schema.json))
- [`fdc3.chat.searchCriteria`](ref/chat/ChatSearchCriteria) ([schema](pathname:///schemas/next/context/chatSearchCriteria.schema.json))
