...
interface AppMetadata extends AppIdentifier {
   ...
   *  instance itself via `fdc3.setInstanceMetadata()`. Must only be set if
   *  `instanceId` is set.
   */
...
interface IAppMetadata : IAppIdentifier
{
    ...
    /// position. May be updated by the instance itself via SetInstanceMetadata. Must only be set if InstanceId is set.
    /// </summary>
...
type AppMetadata struct {
  ...
  // An optional set of metadata fields that can be used to disambiguate
  // instances, such as a window title or screen position. May be updated by the
  // instance itself via SetInstanceMetadata. Must only be set if `instanceId`
  // is set.
...
An application MAY update its own instance metadata via [`fdc3.setInstanceMetadata()`](DesktopAgent#setinstancemetadata), allowing Desktop Agents to display meaningful, instance-specific information (e.g. the name of the contact or instrument currently being viewed). It is returned in the `instanceMetadata` field of an [`AppMetadata`](#appmetadata) object and MUST only be set if `instanceId` is also set.
...
- [`AppMetadata`](#appmetadata)
- [`DesktopAgent.setInstanceMetadata`](DesktopAgent#setinstancemetadata)
