...
Used to clear the specified context type if provided, otherwise, clear all context types present in the channel. When a context type is cleared the Desktop Agent MUST update its internal representation of the context in the channel and ensure that subsequent calls to [`getCurrentContext`](#getcurrentcontext) and, (for *user channels*) any new listeners added to that channel (through [`joinUserChannel`](DesktopAgent#joinUserChannel) or [`addContextListener`](DesktopAgent#addContextListener)) will not receive context of the cleared type until new instance of that context type has been broadcast onto the channel. Hence, *user channel* context listeners registered to receive all types and calls to `getCurrentContext` may receive an older context object if the most recently broadcast type was cleared and will receive no context if all context types were cleared.
