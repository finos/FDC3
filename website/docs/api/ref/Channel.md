...
Desktop Agents MUST also immediately notify the apps that are listening to the `contextCleared` event for this
channel. The event's `channelId` field MUST identify this channel. If a `contextType` parameter was provided, then the
