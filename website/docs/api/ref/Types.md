...
FDC3 API operations make use of several type declarations and return various types of metadata.
...
interface ContextMetadata {
   ...
   *  Agent MUST forward it. If no traceId is provided, the Desktop Agent
   *  MAY generate a new one. */
...
Metadata relating to a context or intent received through the `addContextListener` and `addIntentListener` functions. Includes required delivery information provided by the Desktop Agent (`source`, `timestamp`) and optional metadata forwarded from the originating app or generated by the Desktop Agent (`signature`, `antiReplay`, `traceId`, `custom`).
...
An `IntentHandler` MAY return a [`ContextWithMetadata`](#contextwithmetadata) object instead of a plain `Context` to include app-provided metadata (e.g. a `traceId` or `signature`) alongside the context result. The Desktop Agent will merge this with its own generated metadata and make the combined [`ContextMetadata`](#contextmetadata) available to the raising app via [`IntentResolution.getResultMetadata()`](#intentresolution). [`IntentResolution.getResult()`](#intentresolution) will still return only the `Context` portion.
...
interface IntentResolution {
   ...
   *  result. The metadata always includes Desktop Agent-provided `source` and
   *  `timestamp` fields and may include a `traceId`. When the intent handler
   *  returns a `ContextWithMetadata` result, any app-provided metadata fields
   *  (`traceId`, `signature`, `antiReplay`, `custom`) are merged with the
   *  Desktop Agent-provided fields.
   *
...
Retrieves a promise that will resolve to [`ContextMetadata`](#contextmetadata) for the intent result. The metadata always includes Desktop Agent-provided `source` and `timestamp` fields and may include a `traceId`.

When the intent handler returns a [`ContextWithMetadata`](#contextwithmetadata) result, any app-provided metadata fields (`traceId`, `signature`, `custom`) are merged with the Desktop Agent-provided fields.
