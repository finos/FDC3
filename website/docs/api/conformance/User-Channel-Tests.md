...
- `UCBasicUsage1` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform above test.
- `UCBasicUsage2` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 2,1,3,4,5 to confirm that the order of `joinUserChannel` and `addContextListener` calls doesn't matter.
- `UCBasicUsage3` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 3,4,1,2,5 to confirm that the current context is automatically received on joining a channel.
- `UCBasicUsage4` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 3,4,2,1,5  to confirm that the current context is automatically received on adding a context listener to an already joined a channel.
...
- `UCFilteredUsage1` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform above test.
- `UCFilteredUsage2` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 2,1,3,4,5.
- `UCFilteredUsage3` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 3,4,1,2,5.
- `UCFilteredUsage4` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue) Perform steps in order: 3,4,2,1,5.
...
- `UCFilteredUsage5` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform above test.
- `UCFilteredUsage6` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform above test, except B will join a _different_ channel to A. Check that you _don't_ receive anything.
- `UCFilteredUsageChange` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform above test, except that after joining, **A** changes channel to a _different_ channel via a further call to `fdc3.joinUserChannel`.  Check that **A** does NOT receive anything.
- `UCFilteredUsageUnsubscribe` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform above test, except that after joining, **A** then `unsubscribe()`s from the channel using the `listener.unsubscribe` function. Check that **A** does NOT receive anything.
- `UCFilteredUsageLeave` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform above test, except that immediately after joining, **A** _leaves the channel_, and so receives nothing.
- `UCFilteredUsageNoJoin` ![2.0+](https://img.shields.io/badge/FDC3-2.0+-blue): Perform the above test, but skip step 2 so that **A** does NOT join a channel. Confirm that the _current channel_ for **A** is NOT set before continuing with the rest of the test.  **A** should receive nothing.
...
- `UCArrayContextListeners1`: Perform above test to verify array filtering works correctly.
- `UCArrayNullContext1`: Perform above test, but in step1 use `addContextListener(["fdc3.instrument", null],handler)` to verify arrays with null behave like direct null (receives ALL contexts).
- `UCArrayEmptyContext1`: Attempt to create a context listener with an empty array `addContextListener([],handler)` and verify context is not received.
