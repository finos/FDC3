...
- Notify listeners registered for the `contextCleared` event (via [`addEventListener`](ref/Channel#addeventlistener)) when [`clearContext`](ref/Channel#clearcontext) is called, including the channel's `channelId` and the `contextType` that was cleared, or `null` if all context types were cleared.
