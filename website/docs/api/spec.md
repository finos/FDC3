...
- Accept an optional [`AppProvidableContextMetadata`](ref/Metadata#appprovidablecontextmetadata) argument in [`broadcast`](ref/Channel#broadcast), allowing applications to provide `traceId`, `signature`, `antiReplay` and `custom` metadata with context messages.
...
- Notify listeners registered for the `contextCleared` event (via [`addEventListener`](ref/Channel#addeventlistener)) when [`clearContext`](ref/Channel#clearcontext) is called, including the channel's `channelId` and the `contextType` that was cleared, or `null` if all context types were cleared. Desktop Agents MAY deliver only events for the individual channel on which the listener was registered, or MAY deliver events for all public channels.
