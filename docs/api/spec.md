...
An app queries the current context of the `execution` channel (as current context is not automatically received on adding the listener):

```js
const executionChannel = await fdc3.getOrCreateChannel('execution');
const context = await executionChannel.getCurrentContext('fdc3.instrument');
```
