...
window.addEventListener("load", () => {
    ...
    function changeSize(expanded: boolean) {
        ...
        if (expanded) {
            myPort.postMessage({ type: BrowserTypes.FDC3_USER_INTERFACE_RESTYLE_TYPE, payload: { updatedCSS: DEFAULT_EXPANDED_CSS } } as Fdc3UserInterfaceRestyle)
            ...
            myPort.postMessage({ type: BrowserTypes.FDC3_USER_INTERFACE_RESTYLE_TYPE, payload: { updatedCSS: { ...DEFAULT_COLLAPSED_CSS, ...position } } } as Fdc3UserInterfaceRestyle)
            setTimeout(() => {
