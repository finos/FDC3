...
export class ContextMetadataValidator {
  validateRequiredFields(metadata: ContextMetadata, sourceAppId?: string) {
    expect(metadata, `ContextMetadata was not provided${metadataDocs}`).to.not.be.undefined;
  ...
  validateSignature(metadata: ContextMetadata, expectedSignature: string) {
    expect(metadata, `ContextMetadata did not have property signature${metadataDocs}`).to.have.property('signature');
    expect(metadata.signature, `ContextMetadata.signature did not match expected value`).to.be.equal(expectedSignature);
  }
