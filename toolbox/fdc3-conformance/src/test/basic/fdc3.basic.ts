...
const basicGI2 = (fdc3: DesktopAgent, documentation: string) => {
  it('(BasicGI2) Returns optionalFeatures object', async () => {
    try {
      const info = await fdc3.getInfo();
      expect(info, documentation).to.have.property('optionalFeatures').that.is.an('object');
      expect(info.optionalFeatures, documentation).to.have.property('UserChannelMembershipAPIs').that.is.a('boolean');
      expect(info.optionalFeatures, documentation).to.have.property('DesktopAgentBridging').that.is.a('boolean');
    } catch (ex) {
      handleFail(documentation, ex);
    }
  });
