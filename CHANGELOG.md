...
* Fixed `@finos/fdc3-schema`'s generated `BrowserTypes`/`BridgingTypes` modules pulling their entire `Convert` validation runtime (~240KB) into every consumer's bundle, even ones that only call `getAgent()`. The `Convert` class, its quicktype runtime helpers, and the `isValid<X>` predicates that depend on it are now generated into separate `BrowserTypesValidation`/`BridgingTypesValidation` modules; a new, leaner `BrowserTypesCore`/`BridgingTypesCore` export carries just the message-type interfaces and fast `is<X>` predicates, and `fdc3-get-agent`, `fdc3-agent-proxy` and `fdc3-web-impl` now import that directly instead of the full `BrowserTypes`/`BridgingTypes` namespace. `BrowserTypes`/`BridgingTypes` themselves are unchanged - still a full compatibility barrel re-exporting everything (types, fast predicates, `Convert`, `isValid<X>`) - so no existing code needs to change. All four namespaces (`BrowserTypes`, `BrowserTypesCore`, `BrowserTypesValidation`, and the `Bridging` equivalents) are exported from both `@finos/fdc3-schema` and `@finos/fdc3`. Also added `"sideEffects": false` to all `@finos/*` packages. Reduces a minimal `getAgent()`-only bundle from ~188KB to ~67KB minified (~27KB to ~15KB gzipped), with zero change for consumers still importing the full `BrowserTypes`/`BridgingTypes`. ([#1901](https://github.com/finos/FDC3/issues/1901))
