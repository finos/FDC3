...
* Aligned the optional `AppProvidableContextMetadata` argument of `broadcast`, `open`, `raiseIntent` and `raiseIntentForContext` with a single canonical DACP wire representation: the `metadata` field is now **optional** in the `broadcastRequest`, `openRequest`, `raiseIntentRequest` and `raiseIntentForContextRequest` schemas and MUST be omitted (rather than sent as an empty object) when the app supplies no metadata argument. The reference proxy omits the field accordingly and no longer fabricates a `traceId` on the request — `traceId` generation (alongside `source` and `timestamp`) is the Desktop Agent's responsibility when it enriches the delivered `ContextMetadata`. The DACP documentation now distinguishes message `meta` (transport), `payload.metadata` (app-provided `AppProvidableContextMetadata`) and the enriched `ContextMetadata` produced by the Desktop Agent. It is now a normative requirement that a Desktop Agent MUST provide a `traceId` in the delivered `ContextMetadata` (forwarding an app-provided one, or generating one when absent), so receiving apps can always rely on a `traceId` being present; `ContextMetadata.traceId` is now a required property. ([#2163](https://github.com/finos/FDC3/issues/2163))
...
* Made the DACP `getCurrentContextResponse` `context`/`metadata` relationship a normative, schema-enforced invariant: when `payload.context` is non-null, `payload.metadata` MUST be present and contain the complete `ContextMetadata` for that context; when `payload.context` is `null`, `payload.metadata` MUST be `null`. The JSON Schema now enforces this via a conditional constraint, the DACP documentation states it normatively, and the reference proxy's `getCurrentContextWithMetadata()` no longer fabricates fallback metadata (e.g. an `unknown` source or the response timestamp) — it uses the metadata guaranteed by the invariant, preserving the retained context's actual provenance. ([#2166](https://github.com/finos/FDC3/issues/2166))
...
* **Breaking:** Moved the `Convert` class and `isValid<X>` validation predicates (e.g. `isValidRaiseIntentRequest`) off of `@finos/fdc3-schema`'s `BrowserTypes` and `BridgingTypes` namespaces and into new, separately-exported `BrowserTypesValidation` and `BridgingTypesValidation` namespaces (also re-exported from `@finos/fdc3`). Code using `BrowserTypes.Convert` / `BrowserTypes.isValid<X>` or `BridgingTypes.Convert` / `BridgingTypes.isValid<X>` must update to `BrowserTypesValidation.Convert` / `BrowserTypesValidation.isValid<X>` (and the `BridgingTypesValidation` equivalents). The fast `is<X>` predicates, `<X>_TYPE` constants and plain message-type interfaces are unaffected and remain on `BrowserTypes`/`BridgingTypes`. See "Fixed" below for the bundle-size motivation. ([#1901](https://github.com/finos/FDC3/issues/1901))
...
* Fixed `@finos/fdc3-schema`'s generated `BrowserTypes`/`BridgingTypes` modules pulling their entire `Convert` validation runtime (~240KB) into every consumer's bundle, even ones that only call `getAgent()`. The `Convert` class, its quicktype runtime helpers, and the `isValid<X>` predicates that depend on it are now generated into separate `BrowserTypesValidation`/`BridgingTypesValidation` modules, exported alongside the existing ones from `@finos/fdc3-schema` and `@finos/fdc3`, so bundlers no longer need to include them unless a consumer explicitly imports them. Also added `"sideEffects": false` to all `@finos/*` packages. Reduces a minimal `getAgent()`-only bundle from ~188KB to ~67KB minified (~27KB to ~15KB gzipped). ([#1901](https://github.com/finos/FDC3/issues/1901))
...
* Replaced the webpack build in `toolbox/fdc3-conformance` with a Vite build to align with other packages. ([#1830](https://github.com/finos/FDC3/issues/1830))
