	...
	CIQ.QuoteFeed.BarChart.prototype.fetch = function (params, cb) {
		...
		if (params.endDate) {
			...
			if (params.startDate) {
				...
				params.maxRecords = 0;
			} else if (!params.maxRecords) params.maxRecords = params.maxTicks;
		} else {
			if (params.startDate) {
				startDate = new Date(params.startDate);
				if (CIQ.ChartEngine.isDailyInterval(params.interval)) {
		...
		if (isbats && params.update) {
			if (
				symbol.indexOf(".BZ") == -1 &&
				(!this.isAfterDelayed() || params.extended) &&
				this.batsOpen() &&
				symbol.charAt(0) != "$"
			)
				// After 4:20 always get delayed data on refreshes
				symbol = symbol + ".BZ";
		}
		...
		CIQ.postAjax(url, null, function (status, res) {
			...
			cb({
				...
				attribution: { source: "barchart", exchange: attrExch },
			});
		});
	};
