...
(function (definition) {
	...
	CIQ.QuoteFeed.Xignite.prototype.fetch = function (params, cb) {
		...
		var isIndex =
			this.isIndex(symbol) || _checkAssetClass(params.symbolObject.assetClass, CIQ.QuoteFeed.Xignite.Constants.INDEX);
		...
		var isForex =
			...
			_checkAssetClass(params.symbolObject.assetClass, CIQ.QuoteFeed.Xignite.Constants.CURRENCY);
		var isFuture =
			CIQ.Market.Symbology.isFuturesSymbol(symbol) ||
			_checkAssetClass(params.symbolObject.assetClass, CIQ.QuoteFeed.Xignite.Constants.FUTURES);
		...
		if (myMaxRecords > maxTicks || (params.stx.chart.symbol == params.symbol && params.fetchMaximumBars))
		...
		if (!params.symbol) {
			...
			if (!params.startDate && params.maxRecords) {
				...
				if (isDaily) startDate.setDate(startDate.getDate() - myMaxRecords);
					...
					startDate.setMinutes(
						startDate.getMinutes() - myMaxRecords * (isNaN(params.interval) ? params.period : params.interval)
					);
			} else if (params.startDate) {
				...
				if (CIQ.Market.Symbology.isForexMetal(symbol, true)) {
					...
					if (isDaily) {
						...
						if (params.startDate) {
							...
							args.StartTime =
								CIQ.mmddyyyy(CIQ.yyyymmdd(startDate)) + " " + CIQ.friendlyDate(pStartDate1).split(" ")[1] + ":00";
						}
					} else {
						args.StartTime =
							CIQ.mmddyyyy(CIQ.yyyymmdd(startDate)) + " " + CIQ.friendlyDate(startDate).split(" ")[1] + ":00";
			...
			args = {
				Symbol: symbol,
				StartTime: params.startDate ? CIQ.mmddyyyy(toMarketTime(params.startDate, marketZone)) : CIQ.mmddyyyy(pastDate),
			...
			if (params.stx.quoteDriver.behavior.snapshotRefresh && myDate.getDay() % 6) {
				...
				if (CIQ.QuoteFeed.Xignite.getSnapshotQuote(params, symbol, isDaily, newDT.getTimezoneOffset(), cb, this))
					return;
			}
			...
			if (isDaily) {
				...
				if (params.startDate) {
					...
					if (myDate.getDate() == pStartDate2.getDate()) {
						args.StartTime =
							CIQ.mmddyyyy(CIQ.yyyymmdd(myDate)) + " " + CIQ.friendlyDate(pStartDate2).split(" ")[1] + ":00";
					}
				}
			}
		} else {
			...
			if (!params.update && isDaily) {
				...
				if (isDaily) {
					if (isMutual) {
						if (this.mfUpdate) api = CIQ.clone(CIQ.QuoteFeed.Xignite.Templates.DelayedMF);
					...
					if (!params.startDate && params.maxRecords) {
						...
						startDate = new Date(myDate.getTime());
						startDate.setMinutes(
							startDate.getMinutes() - myMaxRecords * (params.interval == "hour" ? params.period * 60 : params.period)
						);
					} else if (params.startDate) {
						startDate = toMarketTime(params.startDate, marketZone);
					}
		...
		// adjustment override
		var adjMethod = params.stx.quoteDriver.behavior.adjustmentMethod;
		if (adjMethod && !isIndex && api && api.statics) {
			getSplitInfo = false;
			api.statics = api.statics.replace(/AdjustmentMethod=(PriceReturn|All)/, "AdjustmentMethod=" + adjMethod);
		...
		if (api && args && error === "") {
			...
			var mamaCallbackFunction = function (status, res) {
				...
				function processData(quotes, params) {
					...
					if (api.func == "xNAVs" && api.method.as_of) arr.unshift(arr.reverse().pop()); // Historical NAVs return backwards
					var dt = null;
					var stick = false;
					var NYOffsetMap = {};
					for (i = 1; i < arr.length; i++) {
						...
						if (api.results.message) {
							if (fields[fieldNames[api.results.message]] === "") params.error = "";
							else if (
								fields[fieldNames[api.results.message]].indexOf("No match found") === 0 ||
								fields[fieldNames[api.results.message]].indexOf("Unknown") === 0 ||
						...
						if (date === "") {
							if (batsQuote && fields[fieldNames[api.results.market]].indexOf("OTC") > -1)
						...
						if (!isDaily) {
							...
							if (!(hours.endHour == 23 && hours.endMinute == 59)) {
								...
								if (
									...
									(bcdt2.getHours() == hours.endHour && bcdt2.getMinutes() >= hours.endMinute) ||
									bcdt2.getHours() < hours.beginHour ||
									(bcdt2.getHours() == hours.beginHour && bcdt2.getMinutes() < hours.beginMinute)
								)
									continue;
							}
						}
						...
						if (close > 0) {
							...
							if (batsQuote && params.batsVolumeMultiplier)
								newQuotes[newQuotes.length - 1].Volume *= params.batsVolumeMultiplier;
					...
					if (isDaily) {
						if (this.resultsCache[params.xigniteID]) {
							while (
								...
								if (
									results[0].DT < this.resultsCache[params.xigniteID][this.resultsCache[params.xigniteID].length - 1].DT
								)
									results.shift();
								//strip dups
								else if (
									+results[0].DT ==
									+this.resultsCache[params.xigniteID][this.resultsCache[params.xigniteID].length - 1].DT
								) {
						...
						if (
							...
							this.resultsCache[params.xigniteID][this.resultsCache[params.xigniteID].length - 2].DT.getTime() ==
								this.resultsCache[params.xigniteID][this.resultsCache[params.xigniteID].length - 1].DT.getTime()
						) {
				...
				var isUpdate =
					(params.originalState && params.originalState.update) || (!params.originalState && params.update);
			...
			CIQ.postAjax({
				url: CIQ.QuoteFeed.Xignite.Utility.url(api, args, params, this.token, identifierType),
				cb: function (status, res) {
					if (getSplitInfo) {
						...
						CIQ.postAjax({
							url: CIQ.QuoteFeed.Xignite.Utility.url(splitApi, splitArgs, null, this.token, identifierType),
	...
	CIQ.QuoteFeed.Xignite.Templates = {
		...
		HistoricalIndex: {
			...
			statics: null,
			fields: "Message,Values.Date,Values.Last,Values.Open,Values.High,Values.Low,Values.Volume",
			results: {
				message: "Message",
				date: "Values Date",
				time: null,
		...
		HistoricalFuture: {
			...
			statics: null,
			fields: "Message,Quotes.Date,Quotes.Open,Quotes.High,Quotes.Low,Quotes.Last,Quotes.Volume",
			results: {
				message: "Message",
				date: "Quotes Date",
				time: null,
		...
		BATSRealQuote: {
			...
			statics: null,
			fields: "Message,Date,Open,High,Low,Last,Close,Security.MarketIdentificationCode",
			results: {
				message: "Message",
				date: "Date",
		...
		DelayedIndex: {
			...
			statics: null,
			fields: "Message,Value.Date,Value.Volume,Value.Open,Value.High,Value.Low,Value.Last,Value.Close",
			results: {
				message: "Message",
				date: "Value Date",
				time: null, //not needed for daily update
		...
		DividendData: {
			...
			statics: "StartDate=&EndDate=&IdentifierAsOfDate=&CorporateActionsAdjusted=true",
			fields: null,
		},
	};
	...
	CIQ.QuoteFeed.Xignite.getSnapshotQuote = function (params, symbol, isDaily, offset, cb, caller) {
		...
		if (
			...
			params.stx.chart.masterData
				.length /*&&
			!params.stx.highLowBars[params.stx.layout.chartType] &&
			...
			var url = CIQ.QuoteFeed.Xignite.Utility.url(api, { Symbol: symbol }, params, this.token);
			CIQ.postAjax({
				url: url,
				cb: function (status, res) {
					if (status == 200) {
						...
						if (isDaily) nextDate.setDate(nextDate.getDate() + 1);
							...
							nextDate.setMinutes(nextDate.getMinutes() + (isNaN(params.interval) ? params.period : params.interval));
