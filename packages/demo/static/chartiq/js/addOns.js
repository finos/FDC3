...
(function (root, factory) {
	if (typeof define === "function" && define.amd) {
		define(["chartiq"], factory);
	} else if (typeof exports === "object") {
		module.exports = factory(require("chartiq"));
	} else {
		factory(root);
	}
		...
		stx.prepend("updateChartData", function (appendQuotes, chart, params) {
			...
			if (supportedChartType) {
				...
				var linearChart =
					...
					!this.standaloneBars[this.layout.chartType];
	 ...
	 * and by using the periodicity roll up feature instead of fetching new data from a quote feed.
	 ...
	 * @param {number} [params.boundaries.maxTicks] least number of ticks to display before switching
	...
	CIQ.ContinuousZoom.prototype.execute = function (zoomOut) {
		...
		if (
			(!zoomOut && boundaries.maxCandleWidth && layout.candleWidth > boundaries.maxCandleWidth) ||
				...
				for (var i = 0; i < chart.dataSegment.length; i++) {
					...
					if (m.market_def) {
						if (!nextBoundary || nextBoundary <= ds.DT) {
							...
							var filterSession =
								thisSession !== "" && (!this.layout.marketSessions || !this.layout.marketSessions[thisSession]);
	 ...
	 * }
	 *```
	 * Once instantiated, use the slider `display(true/false)` function to add it or remove it from the chart.
	 * Make sure to use the callback to enable the slider on initial load to prevent 'out of sequence' issues.
	 * See examples.
	 *
	 * If using chatIQ webComponents, the slider needs to be created **before** the UI manager (startUI) is called for custom themes to apply.
	 *
	 * Visual Reference:<br>
	...
	CIQ.RangeSlider = function (params) {
		...
		this.drawSlider = function () {
			...
			var pLeft = (self.pixelLeft =
				self.pixelFromTick(left) - (segmentImage[left] ? segmentImage[left].candleWidth / 2 : halfCandle));
			var pRight = (self.pixelRight =
				self.pixelFromTick(right) + (segmentImage[right] ? segmentImage[right].candleWidth / 2 : halfCandle));
		...
		$(subholder).on("mousedown touchstart pointerdown", function (e) {
			...
			var s = $(self);
			s.prop("startDrag", start).prop("startPixelLeft", self.pixelLeft).prop("startPixelRight", self.pixelRight);
		...
		$(subholder).on("mouseup mouseout touchend pointerup", function (e) {
			...
			var s = $(self);
			self.container.style.cursor = "ew-resize";
			s.prop("startDrag", null).prop("needsLeft", false).prop("needsRight", false);
		});
	 ...
	 * 
	 * For example, concatenating the filed name ( ie: 'Jaw' ) with the study name ( ie: 'Alligator' ) is the default behavior of the tooltip for displaying the value title.
	 ...
	 * @param {boolean} [tooltipParams.interpolation] set to true to show the estimated value when there is no data between bars
	...
	CIQ.Tooltip = function (tooltipParams) {
		...
		CIQ.Marker.Tooltip.placementFunction = function (params) {
			...
			for (var i = 0; i < params.arr.length; i++) {
				...
				if (
					CIQ.Marker.Tooltip.sameBar(stx.chart.dataSegment[bar], marker.lastBar) &&
				...
				if (
					parseInt(getComputedStyle(marker.node).width, 10) + stx.chart.panel.left + offset + cw <
		...
		function renderFunction() {
			...
			for (var f = 0; f < fields.length; f++) {
				...
				if (isRecordDate && !useDataZone) name = "displayDate"; // display date is timezone adjusted
				panel = obj.panel;
				yAxis = obj.yAxis;
