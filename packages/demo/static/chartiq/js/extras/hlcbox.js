	...
	CIQ.ChartEngine.prototype.drawHLCBox = function (params) {
		...
		for (var x = 0; x <= quotes.length; x++) {
			...
			var t1 = yAxis.semiLog
				? yAxis.height * (1 - (Math.log(Math.max(Top, 0)) / Math.LN10 - yAxis.logLow) / yAxis.logShadow)
				: (yAxis.high - Top) * yAxis.multiplier;
			var b1 = yAxis.semiLog
				? yAxis.height * (1 - (Math.log(Math.max(Bottom, 0)) / Math.LN10 - yAxis.logLow) / yAxis.logShadow)
				: (yAxis.high - Bottom) * yAxis.multiplier;
			var c1 = yAxis.semiLog
				? yAxis.height * (1 - (Math.log(Math.max(Close, 0)) / Math.LN10 - yAxis.logLow) / yAxis.logShadow)
		...
		if (shadeColor && !CIQ.isTransparent(shadeColor)) {
			...
			for (c = 0; c < cache.length; c++) {
				...
				for (c = 0; c < cache.length; c++) {
					...
					pb = point.bottom;
					if (shaded) {
						context.rect(pl, pc, Math.max(1, pr - pl), yAxis.flipped ? -Math.max(1, pc - pt) : Math.max(1, pb - pc));
					} else {
