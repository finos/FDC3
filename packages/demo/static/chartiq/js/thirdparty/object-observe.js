...
Object.observe ||
	(function (O, A, root) {
		...
		var isArray =
			 ...
			 */
			createMap =
				typeof root.Map === "undefined" || !Map.prototype.forEach
					? function () {
			 ...
			 * @returns {String[]}
			 */
			getProps = O.getOwnPropertyNames
				? (function () {
						...
						try {
							...
							var avoid = (func(inArray).join(" ") + " ")
								.replace(/prototype |length |name /g, "")
								.slice(0, -1)
								.split(" ");
							if (avoid.length)
			...
			doObserve = function (object, handler, acceptList) {
				...
				if (data) setHandler(object, data, handler, acceptList);
				else {
			...
			performPropertyChecks = (function () {
				var updateCheck = getDescriptor
					? function (object, data, idx, except, descr) {
							...
							if (
								"value" in descr &&
								(ovalue === value ? ovalue === 0 && 1 / ovalue !== 1 / value : ovalue === ovalue || value === value)
							) {
								addChangeRecord(
									object,
									data,
				...
				var deletionCheck = getDescriptor
			...
			addChangeRecord = function (object, data, changeRecord, except) {
				data.handlers.forEach(function (hdata) {
					...
					if (
						(typeof except !== "string" || inArray(acceptList, except) === -1) &&
		...
		O.observe = function observe(object, handler, acceptList) {
			...
			if (typeof acceptList === "undefined") acceptList = defaultAcceptList;
		...
		O.unobserve = function unobserve(object, handler) {
			...
			if (hdata && (odata = hdata.observed.get(object))) {
				...
				if (hdata.observed.size === 1 && hdata.observed.has(object)) handlers["delete"](handler);
				else hdata.observed["delete"](object);

				if (odata.data.handlers.size === 1) observed["delete"](object);
