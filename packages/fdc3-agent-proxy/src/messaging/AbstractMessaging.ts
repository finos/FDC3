...
export abstract class AbstractMessaging implements Messaging {
  ...
  async exchange<X>(message: any, expectedTypeName: string, timeoutErrorMessage?: string): Promise<X> {
    ...
    const messageResponseWaiter = this.waitFor<X>(
      m => m.type == expectedTypeName && m.meta.requestUuid == message.meta.requestUuid,
      errorMessage
    );
    this.post(message);
  ...
  private async exchangeValidationWithId<X>(message: any, connectionAttemptUuid: string): Promise<X> {
    const messageResponseWaiter = this.waitFor(m => m.meta.connectionAttemptUuid == connectionAttemptUuid);
    this.post(message);
