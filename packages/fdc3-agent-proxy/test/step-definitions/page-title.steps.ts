...
function countSetInstanceMetadataPosts(world: CustomWorld): number {
  return world.messaging!.allPosts.filter(p => p.type === 'setInstanceMetadataRequest').length;
}
...
When('page title support is connected to a failing agent', async (world: CustomWorld) => {
  const failingAppSupport = {
    setInstanceMetadata: () => Promise.reject(new Error('agent unavailable')),
...
Then('messaging will have no setInstanceMetadata posts', (world: CustomWorld) => {
  expect(countSetInstanceMetadataPosts(world)).toBe(0);
});

Then('messaging will have {int} setInstanceMetadata posts', (world: CustomWorld, count: number) => {
  expect(countSetInstanceMetadataPosts(world)).toBe(count);
});
