Feature: Default Intent Resolver
  ...
  Scenario: App Requests Intent Resolution
    Given I call "{intent-resolver}" with "chooseIntent" using arguments "{appIntents}" and "{instrumentContext}" as "theIntentPromise"
    And I refer to "{result}" as ""
    And we wait for a period of "200" ms
    Given The intent resolver sends an intent selection message
    And I wait for job "theIntentPromise"
  ...
  Scenario: Intent Resolution Cancelled
    ...
    Then I wait for job "{theIntentPromise}"
